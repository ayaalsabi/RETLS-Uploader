...
// ChatHub is mapped on the server root, not under /api.
// TODO: set this to whatever path app.MapHub<ChatHub>("...") actually uses server-side.
 ...
 * the role/token, not a separate user id, so this reads it straight out of
 * the token instead of requiring a LoginScreen change.
 */
...
export default function FileDetailScreen1() {
  ...
  // Mirror so the handlers registered once inside connectionRef don't close
  // over a stale fileId (same pattern as AmanWay's isDriverActiveRef).
  ...
  async function handleSend() {
    ...
    try {
      ...
      // go back to them.
      formData.append("receiverId", String(submission.uploadedByUserId));
